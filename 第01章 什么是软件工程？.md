...
我们发现编程和软件工程之间有三个关键的差别：时间、规模和权衡取舍。在一个软件项目中，工程师需要更多关注时间成本和需求变更。在软件工程中，我们需要更加关注规模和效率，既包括我们生产的软件，也包括生产软件的组织。最终，作为软件工程师，我们被要求做出更复杂的决策，通常是基于对时间和增长的不准确的估计，结果风险更高。

在谷歌，我们有时会说，“软件工程就是与时俱进地编程。”编程无疑是软件工程的一个重要部分：毕竟，编程首先是产生新软件的方式。如果你接受这种区别，那么很明显我们可能需要在编程任务（开发）和软件工程任务（开发、修改、维护）之间进行划分。时间因素为编程增加了一个重要的新维度。立方不是平方，距离不是速度，软件工程也不是编程。

了解时间对程序的影响的一种方法是想一想这个问题：代码的预期生命周期[^1]是多少？答案可能相差100,000倍，而它们都是合理的。生命周期只有几分钟的代码和将会持续几十年的代码都是合理的。通常，生命周期短的代码不受时间的影响。对于一个只需要存活一个小时的程序，你不太可能考虑适配更新的底层库、操作系统（OS）、硬件或语言版本。这些短期系统实际上“只是”一个编程问题，就像在一维空间压缩得足够扁的立方体是正方形一样。随着我们扩大时间维度，允许更长的生命周期，改变显得更加重要。在十年或更长的时间里，大多数程序依赖关系，无论是隐式的还是显式的，都可能发生改变。这一认识是我们区分软件工程和编程的根本。

这种特质是我们所说的软件可持续性的核心。在软件的预期生命周期内，如果你能对任何有价值的变化做出反应，无论是技术还是业务原因，那么你的项目就是可持续的。我们自以为是地只在寻找这样一种能力 —— 因为缺乏价值或有其他更高优先级的事，你可以选择不进行特定的升级[^2]。当你根本无法对底层技术或产品方向的变化做出反应时，你就把高风险赌注押在希望这种变化永远不会成为该软件的关键。对于短期项目，这可能是一个安全的赌注。对于长期的项目（例如几十年），就不是这样了[^3]。

规模是思考软件工程的另一个角度。有多少人参与？随着时间的推移，他们在开发和维护中扮演什么角色？编程通常是个人的创造行为，而软件工程则是团队的协作。早期对软件工程的定义为这一观点提供了一个很好的阐释：“多人开发的多版本程序”[^4]。这暗示了软件工程和编程之间的区别就在于时间和人。团队协作带来了新的问题，但也提供了比任何单个程序员更多的潜力来产生有价值的系统。

团队的组织、项目的组成以及软件项目的行事准则和实行都决定了软件工程复杂性。这些问题是规模所固有的：随着组织的发展和项目的扩大，它在软件产出方面是否变得更加高效？我们的开发流程有没有随着我们的成长而变得更加高效，我们的版本控制原则和测试策略的成本会成比例地增长？从软件工程的早期开始，围绕着沟通和人员规模的议题一直被讨论着，可以追溯到《人月神话》[^5]。这种关于规模的议题通常关系到行事准则，也是软件可持续性问题的基础：做我们需要重复做的事情会花费多少成本？

我们也可以说，软件工程与编程的不同之处在于需要做出的决策的复杂性及其风险。在软件工程中，我们经常被迫在几个路径之间做评估和权衡，有时风险很高价值却不高。软件工程师或项目负责人的工作是针对组织、产品和开发流程的可持续性以及规模成本的管理。记住这几点，权衡利弊，做出合理的决定。有时，我们可能会推迟维护变更，甚至接受扩展性不好的策略，因为我们知道我们会重新审视这些决策。这些选择应该有明确、清晰的递延成本。

假设“这个软件能使用多久”的合理的系数是100,000，那“你的组织中有多少工程师”的系数可能是10,000，“你的项目有多少计算资源可用”更是一个未知数，Google的经验可能与你的不一致。

我们在这种规模下发现的必要的规范大多数也能很好地适用于规模较小的组织：把它当作一份关于软件工程生态系统的报告，我们相信由于您的加入它会变得更好。在某些地方，巨无霸组织有其自身的成本，我们更倾向于避免这额外的开销。我们把它们当作警铃。我们希望，当你的组织壮大到需要考虑这些成本时，你可以找到更好的答案。

在详细了解团队合作、文化、策略和工具之前，我们先阐述一下时间、规模和权衡这些重要主题。

## Time and Change 时间与变化
当新手学习编程时，生成的代码的生命周期通常以小时或天来衡量。编程作业和练习往往都是一次性编写的，几乎没有重构，当然更不需要长期维护。通常，这些程序在初始完成后就不会再重新构建或执行。这在教学环境中并不奇怪。在中学或者高校，也许会有团队项目课程或实践论文。这样的话，这些项目很可能是唯一的生命周期超过一个月的学生代码。他们可能需要重构一些代码，也许是为了应对不断变化的需求，不会有比这更大的变更了。

在常见的行业里也能找到“短期代码”的开发者。移动应用程序的生命周期通常都很短[^6]，不论好坏，全部重写都是相当常见的。初创企业的工程师们可能更应该专注于眼前的目标而不是长期投资：公司可能活不到从回报较慢的基础设施投资中获益的那一天。一个连续创业的开发者可以非常合理地拥有10年的开发经验，并且很少或根本没有维护过生命周期超过一两年的软件的经验。

另一方面，一些成功的项目实际上有无限的生命周期：我们无法准确地预测Google搜索、Linux内核或Apache HTTP服务项目的终点。对于大多数谷歌项目，我们必须假设它们将无限期地存在，我们无法预测何时不再需要升级依赖项、语言版本等。随着他们生命周期的延长，这些长期项目*最终*都会给人一种区别于编程任务或创业开发的感受。

参看图1-1，它展示了处于“预期生命周期”范围两端的两个软件项目。对于从事预期生命周期只有几个小时的任务的程序来说，什么样的维护才是合理的？即，假如你正在编写一个只会执行一次的Python脚本时发现操作系统有了新版本，你会停止编写脚本而升级操作系统吗？当然不会：升级操作系统并不重要。但与此相反，Google搜索停留在20世纪90年代的操作系统版本上显然是一个问题。

![Figure 1-1. Life span and the importance of upgrades][image-1]

从图中可以看出在某一点发生了转变。在一次性项目和持续数十年的项目之间的某个点上发生了转变：项目必须对不断变化的外部因素做出反应的时候[^7]。对于任何一个从一开始就没有升级计划的项目，这种转变可能会非常痛苦，原因有三个，每一个都会使其他原因变得复杂：
- 你正在执行本项目尚未完成的任务；更多隐藏的假设已经融入其中。
- 尝试进行升级的工程师不太可能具有此类任务的经验。
- 升级的规模通常比较大，一次完成几年的升级，而不是增量升级。

因此，在亲身经历过一次这样的升级（或中途放弃）之后，高估后续升级的成本并决定“再也不升级了”是非常合理的。得出这样结论的公司最终会丢弃现有的东西并重写代码，或决定不再升级。有时，与其逃避痛苦的任务（升级），不如选择更负责任的方式，即投入资源以减轻这样的痛苦。这完全取决于升级的成本、升级提供的价值以及相关项目的预期生命周期。

不仅要完成第一次大升级，还要达到能稳定地保持当前状态的要求，这才是项目长期可持续发展的本质。可持续性需要规划和管理必要的变化所产生的影响。我们认为在Google的许多项目中我们已经实现了这种可持续性，主要是通过反复试验。

那么，具体来说，短期编程与编写预期生命周期更长的代码有何不同？随着时间的推移，我们需要更多地意识到“碰巧能行”和“可维护”之间的区别。没有完美的方案能找出这些区别。很不幸，保持软件的长期可维护是一场持久战。

### 海勒姆定律
如果你正在维护一个有其他工程师使用的项目，那么关于“可用”与“可维护”之间最重要的一课就是我们所说的**海勒姆定律**：当一个API有足够多的用户的时候，契约中的承诺并不重要，API中所有可见的行为都会被一些用户直接依赖。

根据我们的经验，在任何关于软件随时间变化的讨论中，这个定律都是一个主导因素。它在概念上类似于熵：对变更和维护随时间变化的讨论必须了解“海勒姆定律”[^8]，正如对效能或热力学的讨论必须知道熵一样。不能因为熵从不减少我们不努力提高效能。同样，在维护软件时，不能因为存在“海勒姆定律”，我们就不制定计划或试图更好地了解它。我们可以减轻它，但我们知道，它永远无法根除。

“海勒姆定律”表达了一种实践知识 —— 即使有最完美的规划、最优秀的工程师和可靠的代码评审机制，我们也无法保证能完全遵守已发布的契约或最佳实践。作为API所有者，你可以通过清晰的接口定义获得一定的灵活性和自由度，但实际上，特定变更的复杂度和难易程度还取决于用户对你的API的依赖程度。如果用户不依赖这些东西，那么你的API将很容易更改。如果有足够多的用户经过足够长的时间，即使是最无害的变更也会破坏某些东西[^9]；你对该变更的价值分析必须考虑到调研、识别和解决那些意外的损害。

### 举例：哈希排序
看一个哈希迭代排序的例子。如果我们将五个元素插入到一个基于散列的集合中，我们会以什么顺序遍历它们？
...
大多数程序员都知道哈希表不是明显有序的。但很少有人知道他们使用的特定哈希表是否会永远提供特定的顺序。这似乎很不起眼，但在过去的一二十年中，它在计算行业内的使用已发生了变化：
- 哈希泛洪攻击[^10]为非确定性哈希迭代提供了更多的激励。
- 研究改进的散列算法或散列容器的潜在效率需要改变哈希迭代顺序。
- 根据海勒姆定律，如果可以，程序员会编写依赖于哈希表遍历顺序的程序。
...
[^7]:	你的优先级和品味会告诉你什么时候应该转变。我们发现，大多数项目似乎愿意在5年内升级。一般来说，5到10年似乎是这一转变的保守估计。

[^8]:	值得称道的是，海勒姆非常谦虚地称其为“隐性依赖法则”，但“海勒姆定律”是Google大多数人都使用的简称。

[^9]:	见 “工作流”，一幅xkcd漫画。

[^10]:	一种DoS攻击，不受信任的用户知道哈希表的结构和散列函数，并以降低表操作的算法性能的方式提供数据。
